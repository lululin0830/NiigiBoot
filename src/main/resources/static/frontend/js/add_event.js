...
const addProduct = function () {
    ...
    fetch('http://localhost:8080/Niigi/AllProductSpecManage', {
				method: 'POST',
				body: JSON.stringify({registerSupplier: registerSupplier}),
				headers: {
        ...
        fetch('http://localhost:8080/Niigi/AddEvent/generateOrderId')
        .then(response => {
            if (!response.ok) {
                throw new Error('Network response was not ok');
            }
...
registerButton.addEventListener('click', async () => {
  const newEvent = JSON.stringify({
    ...
    const response = await fetch('http://localhost:8080/Niigi/AddEvent/addEvent', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
    ...
    const sendCheckboxData = async (eventId, productId) => {
      try {
        await fetch('http://localhost:8080/Niigi/AddEvent/addProduct', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
